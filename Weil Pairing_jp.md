...
$P$ の $i$ 倍点、$P$ の $j$ 倍点を表す関数の因子は、それぞれ次のようになる。

$$[f_{iP}] = i[P+R_1] - i[R_1] - [iP] + [O]$$

$$[f_{jP}] = j[P+R_1] - j[R_1] - [jP] + [O]$$

この $f_{iP}$ と $f_{jP}$ を掛け算した場合の因子は、

$$[{f_{iP}f_{jP}}] = i[P+R_1] - i[R_1] - [iP] + [O] + j[P+R_1] - j[R_1] - [jP] + [O]$$

$$= (i+j)[P+R_1] - (i+j)[R_1] - [iP] - [jP] + 2[O]$$

次に、$iP$ と $jP$ を通る直線の関数の因子は、

$$[l_{iP,jP}] = [iP] + [jP] + [-(i+j)P] - 3[O]$$

さらに、$[i+j]P$ 点を通る垂線の関数の因子は、

$$[v_{(i+j)P}] = [(i+j)P] + [-(i+j)P] - 2[O]$$

ここで、次の合成された有理関数の因子 $[f_{iP}f_{jP}\dfrac{l_{iP,jP}}{v_{(i+j)P}}]$ を計算してみる。
...
一方で、$P$ の $(i+j)$ 倍点を表す関数の因子は、定義より、

$$[f_{(i+j)P}] = (i+j)[P+R_1] - (i+j)[R_1] - [(i+j)P] + [O]$$

この結果、 $[f_{iP}f_{jP}\dfrac{l_{iP,jP}}{v_{(i+j)P}}]$ と $[f_{(i+j)P}]$ が等しいことが分かる。
...
この式から、 $f_{1P}$ を知ることができれば、再帰計算で、 $f_{nP}$ を計算できることがわかる


### 3.6 $f_{1P}$ を求める
※正確には、 $f_{1P}(A_Q)$ を求める必要があります。
...
一方で、点 $P$ と点 $R_1$ を通る直線 $l_{P,R_1}$ の因子は、

$$[l_{P,R_1}] = [P] + [R_1] + [-(P+R_1)] - 3[O]$$

また、点 $[P+R_1]$ を通る垂線 $v_{P+R_1}$ の因子は、

$$[v_{P+R_1}] = [P+R_1] + [-(P+R_1)] - 2[O]$$

ここで、垂線 $v$ を直線 $l$ で割った有理関数の因子を計算すると、

$$[\dfrac{v_{P+R_1}}{l_{P,R_1}}] = [P+R_1] - [R_1] - [P] + [O]$$

となり、 $[f_{1P}]$ と同じになります。すなわち、

$$f_{1P} = \dfrac{v_{P+R_1}}{l_{P,R_1}}$$

正確に書けば、因子 $A_Q$ の値を入れて評価するので、
...
直線 $v_{(i+j)P}$ は、楕円曲線上の $[i+j]P$ 点を通る垂線です。

$T = iP + jP$ なる点 $T$ を楕円曲線の加法で求めておけば、垂線の公式は次のようになります。
...
この関数 $v$ に点 $Q$ を代入して評価したい場合は、次のようになります。

**$$v(Q) = (iP + jP).x - Q.x$$**

このように、関数 $l(Q), v(Q)$ は、実際の各点の座標から計算することができます。
