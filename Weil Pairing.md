...
$P$ の $i$ 倍点、$P$ の $j$ 倍点を表す関数の因子は、それぞれ次のようになります。
...
ここで、次の合成された有理関数の因子 $[f_{iP}f_{jP}\dfrac{l_{iP,jP}}{v_{(i+j)P}}]$ を計算してみます。
...
この結果、 $[f_{iP}f_{jP}\dfrac{l_{iP,jP}}{v_{(i+j)P}}]$ と $[f_{(i+j)P}]$ が等しいことが分かります。
