...
* この分数の計算値が、Weil Pairing値 $\underline{e(P,Q)}$ となる  

ただし
* $P,Q$ は、それぞれ<u><b>別の</b></u> $n$ ねじれグループ上にある必要がある  
...
&=\dfrac{v_{(P+R_1)}([Q+R_2])}{l_{P,R_1}([Q+R_2])}*\dfrac{l_{P,R_1}([R_2])}{v_{(P+R_1)}([R_2])}\\
\end{align*}
$$
略して記すと
$$\begin{align*}
f_{1P}(A_Q)&=\dfrac{v([Q+R_2])}{l([Q+R_2])}*\dfrac{l([R_2])}{v([R_2])}\\
&=\dfrac{v(Q+R_2)}{l(Q+R_2)}*\dfrac{l(R_2)}{v(R_2)}
\end{align*}
$$ 
※ 因子 $[Q+R_2],[R_2]$は、それぞれ点まで分解されている因子ため、点を評価する有理関数においては、因子ではなく「点の値」を使うことができる。<br>
これにより、
$P,Q,R_1,R_2$の座標値$x,y$は分かっているので、$f_{1P}(A_Q)$の値は実計算により求めることができる
* $P,R_1$の$x,y$値から関数 $l,v$ は求められる。
* $(Q+R_2)$の座標は、楕円曲線の加法により求められる。
* 関数$l,v$に、２つの点$(Q+R_2),R_2$の座標$x,y$値を代入して計算できる。
